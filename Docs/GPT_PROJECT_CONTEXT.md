...
- The temporary comparison examples and the legacy Toolbox enemy pack were removed after the owner accepted the procedural animation direction.
- `ServerStorage.EnemyTemplates.CodexMobCollection` contains the retained 20 animated mob templates and four boss templates. They stay hidden and server-only until `EnemyFactory` clones them into a wave. `DebugMobPreviewOrder` drives a single cycle button for roster review; dedicated buttons spawn each boss.
- Active wave weights contain only the 20 retained roster variants; legacy Walker/Troll/example variants remain excluded from waves and debug load tests.
